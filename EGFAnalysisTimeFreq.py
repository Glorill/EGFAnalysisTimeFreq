...
class gfcn_analysis:
    ...
    def __init__(self, DataFileName , isEGF = True,
        ...
        self.NumCtrT = int((self.EndT - self.StartT) / self.DeltaT) + 1
        self.TPoint = np.linspace(self.StartT, self.EndT, self.NumCtrT)

        # the number of velocity points
        self.NumCtrV = int((self.EndV - self.StartV) / self.DeltaV) + 1
    ...
    def GroupVelocityImgCalculate(self):
        ...
        for ii in range(1, self.NumCtrT):
        ...
        for i in range(self.NumCtrT):
            ...
            GroupVelocityImg.append(interpolate.interp1d
                                    (TravPtV[::-1], (EnvelopeImageSignal[i, self.StartWin:self.EndWin+2]/AmpS_T[i])[::-1], kind='cubic', bounds_error=False,fill_value=0)(self.VPoint))
    ...
    def AutoGroupDisperPick(self,minlamdaRatio=2):
        ...
        T_try_index = np.round(np.array(range(11))/10*(self.TPoint[nMaxT-1] - self.TPoint[0])/dT).astype(int)
...
if __name__ == '__main__':
    ...
    gfcn = gfcn_analysis(FilePath, isEGF=False,DeltaV=0.005)
